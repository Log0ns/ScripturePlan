...
type Props = {
  ...
  onUpdate: (updates: Partial<CustomTile>) => void;
  onDelete: () => void;
  onClose: () => void;
};

export default function CustomTileSettings({ tile, onSave, onUpdate, onDelete, onClose }: Props) {
      ...
      <div className="max-w-md mx-auto p-6 pb-8">
        ...
        <div className="flex items-center mb-5">
          <input
            ...
            checked={tile.activeToday || false}
            onChange={(e) => onUpdate({ activeToday: e.target.checked })}
          />
          <label htmlFor="activeToday" className="text-slate-300 text-sm">
            Marked as active today
          </label>
        </div>
