...
export default function ThemesTab({ themes, timeOfDay, onChange }: Props) {
  ...
  const completedDot = `w-2 h-2 rounded-full flex-shrink-0 ${isNight ? 'bg-amber-400' : 'bg-emerald-500'}`;
  const emptyDot = `w-2 h-2 rounded-full flex-shrink-0 ${isNight ? 'bg-slate-600' : 'bg-slate-300'}`;
  ...
  if (view.kind === 'chapters') {
    const book = BIBLE_BOOKS[view.bookIndex];
    const filled = filledCount(view.bookIndex);
    return (
      <div>
        <button onClick={goBack} className={`flex items-center gap-1 mb-4 ${isNight ? 'text-slate-400' : 'text-slate-500'}`}>
          <ArrowLeft className="w-4 h-4" />
        ...
        <div className={`text-xs mb-4 ${isNight ? 'text-slate-500' : 'text-slate-400'}`}>
          {filled} / {book.chapters} chapters
        </div>

        <div className="grid grid-cols-4 gap-2">
          {Array.from({ length: book.chapters }, (_, i) => i + 1).map(ch => {
            const filled = chapterFilled(view.bookIndex, ch);
            return (
              <button
                key={ch}
                ...
                className={`py-2 rounded-xl text-sm font-medium transition-colors
                  ${filled
                    ? isNight ? 'bg-amber-400/20 text-amber-300' : 'bg-emerald-100 text-emerald-700'
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              <div className={complete ? completedDot : emptyDot} />
              <span className={`text-sm font-medium ${isNight ? 'text-slate-200' : 'text-slate-700'}`}>
                {book.name}
              </span>
            </div>
