...
import { MemoryTile, CompletedChunks } from '../types';
...
export default function MemoryTileModal({ tile, allTiles, completedChunks, onUpdate, onDelete, onClose }: Props) {
          ...
          <div className="max-w-md mx-auto p-6 pb-8">
            ...
            <div className="space-y-2">
              {MEMORY_CHUNKS.map((c, idx) => {
                ...
                const activeDone = existingDay >= 30;
                const everCompleted = completedChunks.includes(idx);
                return (
                  <button
                    key={idx}
                  ...
                  >
                    ...
                    <span className={`text-xs ${activeDone || everCompleted ? 'text-emerald-400' : 'text-slate-400'}`}>
                      {activeDone || everCompleted
                        ? '✓ done'
                        : existing ? `day ${existingDay}` : ''}
                    </span>
                  </button>
                );
              })}
