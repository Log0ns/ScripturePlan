...
import { ScriptureIcon, CustomTile, MemoryTile, Themes } from './types';
...
export default function Planny() {
  ...
  const [themes, setThemes] = useLocalStorage<Themes>('planny-themes', {});
  ...
  const syncData = useMemo(() => ({ icons, customTiles, memoryTiles, themes, daysCompleted, lastResetDate }), [icons, customTiles, memoryTiles, themes, daysCompleted, lastResetDate]);
  useFirebaseSync(user, syncData, { setIcons, setCustomTiles, setMemoryTiles, setThemes, setDaysCompleted, setLastResetDate });
  ...
  const handleMemoryTap = useCallback((tile: MemoryTile) => {
    ...
    setMemoryTiles(prev => prev.map(t =>
      t.id === tile.id ? { ...t, day: Math.min(t.day + 1, 30) } : t
    ));
  }, [online]);
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        <div className="px-6 pt-12 pb-8 max-w-md mx-auto relative">
          ...
          <div className="flex gap-6 mb-6 overflow-x-auto">
            <button className={tabLabelClass('reading')} onClick={() => setTab('reading')}>Reading</button>
            <button className={tabLabelClass('memorization')} onClick={() => setTab('memorization')}>Memorization</button>
            <button className={tabLabelClass('themes')} onClick={() => setTab('themes')}>Themes</button>
            <button className={tabLabelClass('prayer')} onClick={() => setTab('prayer')}>Prayer</button>
          </div>
