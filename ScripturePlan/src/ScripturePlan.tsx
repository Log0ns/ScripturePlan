...
import { ReadingTile, PrayerTile, SettingsModal, GlobalSettingsModal, CustomTileSettings, Toast } from './components';

export default function ScripturePlan() {
  const [icons, setIcons] = useLocalStorage<ScriptureIcon[]>('scripture-icons', DEFAULT_ICONS);
  ...
  const handleTap = useCallback((icon: ScriptureIcon) => {
    ...
    setToast(`${BIBLE_BOOKS[advanced.bookIndex].name} ${advanced.chapter}`);
  }, []);
        ...
        <div className="px-6 pt-12 pb-8 max-w-md mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {icons.map(icon => (
                <ReadingTile key={icon.id} icon={icon} timeOfDay={timeOfDay} onTap={handleTap} onLongPress={handleLongPress} />
              ))}
              {icons.length < 10 && (
