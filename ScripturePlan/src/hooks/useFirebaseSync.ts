...
export function useFirebaseSync(
  ...
  // Pull from Firestore once per session on sign-in
  useEffect(() => {
    if (!user) {
    ...
    getDoc(ref).then(snap => {
      ...
      console.error(err);
      // Don't enable push if pull failed — prevents overwriting cloud with defaults
      hasPulled.current = false;
    });
  }, [user]);
  ...
  // Push current data when coming back online, or retry pull if it failed
  useEffect(() => {
    if (!user) return;
    const handleOnline = () => {
